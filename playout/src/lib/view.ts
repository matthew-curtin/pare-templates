 ...
 * and BOTH its `ready` and `finished` promises reject with an
 * InvalidStateError. Nobody is awaiting them, so they surface as
 * uncaught rejections in the console — twice per collision. It is not a
 * bug in the feature: an aborted transition is the correct behaviour
 * when somebody clicks two hours in quick succession, and the second
 * transition runs fine. But a console with red in it is a console people
 * stop reading, and this one is only visible if you actually drive the
 * interaction rather than look at it (CONVENTIONS §8).
 */
